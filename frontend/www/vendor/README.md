...
| `output/chtml/fonts/woff-v2/*.woff` (23 files) | 3.2.2 (matches the bundle above) | https://cdn.jsdelivr.net/npm/mathjax@3.2.2/es5/output/chtml/fonts/woff-v2/ | Apache-2.0 ([MATHJAX_LICENSE.md](MATHJAX_LICENSE.md)) | not individually recorded — re-download from the URL above if in doubt |
...
Instead, `results-example.html` sets `enableExplorer: false` and
`enableEnrichment: false` in its `window.MathJax` config, before loading the
script, which keeps the Explorer menu item from ever activating and
triggering that load. If MathJax's accessibility features are wanted in the
future, vendor the SRE package chain at that point rather than re-enabling
this against the CDN.
